...
| 2뎁스 | `YYYYMMDD` | `20260421` |
| 3뎁스 | 작업목적 (스네이크케이스) | `pms_이관`, `predict_power_io_계약` |
...
```
docs/plan/20260421/pms_이관/PLAN1.md
docs/tasks/20260421/pms_이관/TASK1.md
docs/results/20260421/pms_이관/RESULT1.md
docs/reviews/20260421/pms_이관/REVIEW1.md
```
...
- 예: `docs/plan/20260421/pms_이관/PLAN2.md`
...
## 모듈 레이아웃
<!-- alg/<module>/ 하위 새 파일이 있으면 작성. module-layout.md 원칙 준수 -->
## 테스트 전략
## 제외 사항
## 예상 산출물
...
| 규칙 | 설명 |
|------|------|
| **전체 상대 경로 필수** | `algorithm/` 기준 전체 경로를 백틱으로 감싸 기록한다 |
| **축약 경로 금지** | `alg/pms/.../...` 형태 금지 |
| **글롭 패턴 금지** | `alg/pms/core/*.py` 형태 금지 |
| **명령어 별도 표기** | `python -m pytest`, `ruff check` 등 실행 명령어는 코드블록 또는 일반 텍스트로 기록 |

```markdown
# 올바른 예
- [ ] `alg/pms/__main__.py` 생성
- [ ] `alg/pms/cli.py` 생성
- [ ] `alg/pms/core/diagnosis.py` 수정

# 잘못된 예 (훅 파싱 오류 유발)
- [ ] `alg/pms/.../diagnosis.py` 생성   ← 축약 금지
- [ ] `alg/pms/core/*.py` 수정          ← 글롭 금지
- [ ] `python -m pytest alg/pms/` 실행  ← 명령어 금지
```
...
```markdown
### Phase 4: 검증
- [ ] `ruff check alg/pms/` 실행 성공 확인
- [ ] `python -m pytest alg/pms/` 실행 성공 확인
```

> `./` 로 시작하거나 `pytest`·`gradlew` 를 포함하는 항목은 훅이 자동으로 명령어로 인식하여 스킵한다.
