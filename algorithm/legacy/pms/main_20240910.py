...
def main():
    ...
    while True:
        try:
            ...
            test_acq_date_time = '2024-08-24 11:34:06.000'
            ...
            for acq_datetime in acq_date_rows:
                ...
                if not on_rows:
                    on_tags['pump_scada_01'] = 0
                    on_tags['pump_scada_02'] = 0
                    on_tags['pump_scada_03'] = 1
                    on_tags['pump_scada_04'] = 0
                    on_tags['pump_scada_05'] = 0
                    on_tags['pump_scada_06'] = 0
                ...
                for i, motor_sensor_name in enumerate(motor_sensor_list):
                    ...
                    for row in rows:
                        ...
                        if config.get_mapping_id(row[3]) in motor_sensor_name:
                            ...
                            tsql = f"INSERT INTO TB_TIMEWAVE(MOTOR_ID, CHANNEL_ID, ACQ_DATE, DATA_ARRAY) VALUES( '{motor_id}', '{motor_sensor_name}', '{acq_datetime[0]}', '{compress_array(y_list)}' ) "
                            #print(tsql)
                            conn.insert(tsql)
                            print('##################')
                    ...
                    #RMS 추출
                    print("RMS")
                    rSql = f"INSERT INTO TB_RMS(MOTOR_ID, CHANNEL_ID, ACQ_DATE, RMS) VALUES( '{motor_id}', '{motor_sensor_name}', '{acq_datetime[0]}', '{device_dict[motor_sensor_name].v_rms}' ) "
                    conn.insert(rSql)
                    #RMS 추출
                    ...
                    spSql = f"INSERT INTO TB_SPECTRUM(MOTOR_ID, CHANNEL_ID, ACQ_DATE, DATA_ARRAY) VALUES('{motor_id}', '{motor_sensor_name}', '{acq_datetime[0]}', '{compress_array(y_list)}')"
                    conn.insert(spSql)
                    #스펙트럼 데이터
                    
                    print("FREQ")
                    #1x device_dict[motor_sensor_name].nxs_freq[1]
                    f1xSql = f"INSERT INTO TB_FREQ(MOTOR_ID, CHANNEL_ID, ACQ_DATE, FREQ_TYPE, FREQ_VALUE) VALUES('{motor_id}', '{motor_sensor_name}', '{acq_datetime[0]}', '1', '{device_dict[motor_sensor_name].nxs_freq[1]}')"
                    conn.insert(f1xSql)
                    
                    #2x device_dict[motor_sensor_name].nxs_freq[1]
                    f2xSql = f"INSERT INTO TB_FREQ(MOTOR_ID, CHANNEL_ID, ACQ_DATE, FREQ_TYPE, FREQ_VALUE) VALUES('{motor_id}', '{motor_sensor_name}', '{acq_datetime[0]}', '2', '{device_dict[motor_sensor_name].nxs_freq[2]}')"
                    conn.insert(f2xSql)
                    
                    #3x device_dict[motor_sensor_name].nxs_freq[1]
                    f3xSql = f"INSERT INTO TB_FREQ(MOTOR_ID, CHANNEL_ID, ACQ_DATE, FREQ_TYPE, FREQ_VALUE) VALUES('{motor_id}', '{motor_sensor_name}', '{acq_datetime[0]}', '3', '{device_dict[motor_sensor_name].nxs_freq[3]}')"
                    conn.insert(f3xSql)
                ...
                for i, pump_sensor_name in enumerate(pump_sensor_list):
                    ...
                    for row in rows:
                        if config.get_mapping_id(row[3]) in pump_sensor_name:
                            ...
                            df['y'] = df['y'].apply(lambda x: round(x, 10))
                            y_list = ["{:.10f}".format(float(value)) for value in df['y']]
                            tsql = f"INSERT INTO TB_TIMEWAVE(MOTOR_ID, CHANNEL_ID, ACQ_DATE, DATA_ARRAY) VALUES( '{motor_id}', '{pump_sensor_name}', '{acq_datetime[0]}', '{compress_array(y_list)}' ) "
                            conn.insert(tsql)
                            #타임웨이브
                    ...
                    #RMS 추출
                    rSql = f"INSERT INTO TB_RMS(MOTOR_ID, CHANNEL_ID, ACQ_DATE, RMS) VALUES( '{motor_id}', '{pump_sensor_name}', '{acq_datetime[0]}', '{device_dict[pump_sensor_name].v_rms}' ) "
                    conn.insert(rSql)
                    #RMS 추출
                    ...
                    spSql = f"INSERT INTO TB_SPECTRUM(MOTOR_ID, CHANNEL_ID, ACQ_DATE, DATA_ARRAY) VALUES('{motor_id}', '{pump_sensor_name}', '{acq_datetime[0]}', '{compress_array(y_list)}')"
                    conn.insert(spSql)
                    #스펙트럼 데이터
                    
                    #1x device_dict[pump_sensor_name].nxs_freq[1]
                    f1xSql = f"INSERT INTO TB_FREQ(MOTOR_ID, CHANNEL_ID, ACQ_DATE, FREQ_TYPE, FREQ_VALUE) VALUES('{motor_id}', '{pump_sensor_name}', '{acq_datetime[0]}', '1', '{device_dict[pump_sensor_name].nxs_freq[1]}')"
                    conn.insert(f1xSql)
                    
                    #2x device_dict[pump_sensor_name].nxs_freq[1]
                    f2xSql = f"INSERT INTO TB_FREQ(MOTOR_ID, CHANNEL_ID, ACQ_DATE, FREQ_TYPE, FREQ_VALUE) VALUES('{motor_id}', '{pump_sensor_name}', '{acq_datetime[0]}', '2', '{device_dict[pump_sensor_name].nxs_freq[2]}')"
                    conn.insert(f2xSql)
                    
                    #3x device_dict[pump_sensor_name].nxs_freq[1]
                    f3xSql = f"INSERT INTO TB_FREQ(MOTOR_ID, CHANNEL_ID, ACQ_DATE, FREQ_TYPE, FREQ_VALUE) VALUES('{motor_id}', '{pump_sensor_name}', '{acq_datetime[0]}', '3', '{device_dict[pump_sensor_name].nxs_freq[3]}')"
                    conn.insert(f3xSql)
                print("end pump")
