...
def main():
    ...
    while True:
        try:
            ...
            #수동 시간 조절
            now_acq_date_time = '2024-03-06 14:00:00'
            ...
            cur.execute(
                f"SELECT acq_date from TB_MOTOR where proc_stat = 0 group by acq_date order by acq_date desc;"  ### sim TB_MOTOR
            )
            '''
            cur.execute(
            ...
            for acq_datetime in acq_date_rows:
                ...
                cur.execute(
                    f"SELECT motor_id, equipment_id, center_id, channel_id, data_array from TB_MOTOR where DATE_FORMAT(ACQ_DATE, '%Y-%m-%d %H:%i:%S') >= DATE_SUB(DATE_FORMAT('{acq_datetime[0]}','%Y-%m-%d %H:%i:%S'), INTERVAL 1 MINUTE );"  ### sim TB_MOTOR
                )
                rows = cur.fetchall()
                end = time.time()
                row_count = len(rows)
