...
public interface DatasetGroupRepository extends GroupBaseRepository <DatasetGroup>, DatasetGroupCustomRepository {
    @Query(value = """
    WITH RECURSIVE r AS (
        SELECT 
            g.grp_id,
    ...
    FROM r
    """, nativeQuery = true)
    List<DatasetGroup> findAllGroupRecursive(@Param("grpIds") List<String> grpIds);
}
