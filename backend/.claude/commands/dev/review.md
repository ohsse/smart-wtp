...
- [ ] **도메인 규칙 정합성**: 알람 4단계(0:정상·1:주의·2:경보·3:TRIP) 처리 누락 여부, 인터록 조건 구현 여부, 운전 모드 전환 로직 누락 여부 (`domain-glossary.md` 기준)
- [ ] **쿼리 성능**: N+1 패턴 없음, 대용량 테이블 페이지네이션 적용, 시계열 쿼리 파티션 키 포함 (`db-patterns.md` 기준)
- [ ] **레거시 동등성**: PLAN 문서의 도메인 모델이 KWS-GS-*-AN04 요구사항 추적표에 매핑 가능한지, 레거시(EMS/PMS) 대비 누락 기능이 없는지 (`legacy-mapping.md` 기준)
